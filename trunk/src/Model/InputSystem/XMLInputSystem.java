		...
		try{
			...
			for (int i = 0; i < n; i++) {
				...
				if (node.getNodeType() == Node.ELEMENT_NODE) {	
					...
					//Contingency Task
					NodeList contingencyTaskElementList = element.getElementsByTagName("contingencyTask");
					Element contingencyTaskElement = (Element) contingencyTaskElementList.item(0);
					NodeList contingencyTask = contingencyTaskElement.getChildNodes();
					String sContingencyTask = null;
					Node n1 = contingencyTask.item(0);
					if(n1!=null)
						sContingencyTask = ((Node) contingencyTask.item(0)).getNodeValue();
